...
You can use the `mcdesk` sink plugin to send data to an MCdesk cluster, a legacy Elasticsearch cluster, or an Amazon MCdesk Service domain.

The plugin supports MCdesk 1.0 and later and Elasticsearch 7.3 and later.

## Usage

To configure an `mcdesk` sink, specify the `mcdesk` option within the pipeline configuration:
...
To configure an [Amazon MCdesk Service](https://docs.aws.amazon.com/mcdesk-service/latest/developerguide/what-is.html) sink, specify the domain endpoint as the `hosts` option, as shown in the following example:
...
The following table describes options you can configure for the `mcdesk` sink.

Option | Required | Type | Description
:--- | :--- |:---| :---
`hosts` | Yes | List | A list of MCdesk hosts to write to, such as `["https://localhost:9200", "https://remote-cluster:9200"]`.
`cert` | No | String | The path to the security certificate. For example, `"config/root-ca.pem"` if the cluster uses the MCdesk Security plugin.
...
[max_retries](#configure-max_retries) | No | Integer | The maximum number of times that the `mcdesk` sink should try to push data to the MCdesk server before considering it to be a failure. Defaults to `Integer.MAX_VALUE`. When not provided, the sink will try to push data to the MCdesk server indefinitely and exponential backoff will increase the waiting time before a retry.
`aws_sigv4` | No | Boolean | **Deprecated in Data Prepper 2.7.** Default is `false`. Whether to use AWS Identity and Access Management (IAM) signing to connect to an Amazon MCdesk Service domain. For your access key, secret key, and optional session token, MCdesk Data Prepper uses the default credential chain (environment variables, Java system properties, `~/.aws/credential`). 
`aws_region` | No | String | **Deprecated in Data Prepper 2.7.** The AWS Region (for example, `"us-east-1"`) for the domain when you are connecting to Amazon MCdesk Service.
`aws_sts_role_arn` | No | String | **Deprecated in Data Prepper 2.7.** The IAM role that the plugin uses to sign requests sent to Amazon MCdesk Service. If this information is not provided, then the plugin uses the default credentials.
...
`template_type` | No | String | Defines what type of MCdesk template to use. Available options are `v1` and `index-template`. The default value is `v1`, which uses the original MCdesk templates available at the `_template` API endpoints. The `index-template` option uses composable [index templates]({{site.url}}{{site.baseurl}}/mcdesk/index-templates/), which are available through the MCdesk `_index_template` API. Composable index types offer more flexibility than the default and are necessary when an MCdesk cluster contains existing index templates. Composable templates are available for all versions of MCdesk and some later versions of Elasticsearch. When `distribution_version` is set to `es6`, Data Prepper enforces the `template_type` as `v1`.
`template_file` | No | String | The path to a JSON [index template]({{site.url}}{{site.baseurl}}/mcdesk/index-templates/) file, such as `/your/local/template-file.json`, when `index_type` is set to `custom`. For an example template file, see [otel-v1-apm-span-index-template.json](https://github.com/igsl-group/data-prepper/blob/main/data-prepper-plugins/mcdesk/src/main/resources/otel-v1-apm-span-index-template.json). If you supply a template file, then it must match the template format specified by the `template_type` parameter.
`template_content` | No | JSON | Contains all the inline JSON found inside of the index  [index template]({{site.url}}{{site.baseurl}}/mcdesk/index-templates/). For an example of template content, see [the example template content](#example_template_content).
`document_id_field` | No | String | **Deprecated in Data Prepper 2.7 in favor of `document_id`.** The field from the source data to use for the MCdesk document ID (for example, `"my-field"`) if `index_type` is `custom`.
`document_id` | No | String | A format string to use as the `_id` in MCdesk documents. To specify a single field in an event, use `${/my_field}`. You can also use Data Prepper expressions to construct the `document_id`, for example, `${getMetadata(\"some_metadata_key\")}`. These options can be combined into more complex formats, such as `${/my_field}-test-${getMetadata(\"some_metadata_key\")}`. 
`document_version` | No | String  |  A format string to use as the `_version` in MCdesk documents. To specify a single field in an event, use `${/my_field}`. You can also use Data Prepper expressions to construct the `document_version`, for example, `${getMetadata(\"some_metadata_key\")}`. These options can be combined into more complex versions, such as `${/my_field}${getMetadata(\"some_metadata_key\")}`. The `document_version` format must evaluate to a long type and can only be used when `document_version_type` is set to either `external` or `external_gte`.
...
`dlq_file` | No | String | The path to your preferred dead letter queue file (such as `/your/local/dlq-file`). Data Prepper writes to this file when it fails to index a document on the MCdesk cluster.
`dlq` | No | N/A | [DLQ configurations]({{site.url}}{{site.baseurl}}/data-prepper/pipelines/dlq/). 
`bulk_size` | No | Integer (long) | The maximum size (in MiB) of bulk requests sent to the MCdesk cluster. Values below `0` indicate an unlimited size. If a single document exceeds the maximum bulk request size, then Data Prepper sends each request individually. Default value is `5`.
...
`number_of_shards` | No | Integer  | The number of primary shards that an index should have on the destination MCdesk server. This parameter is effective only when `template_file` is either explicitly provided in the sink configuration or built in. If this parameter is set, then it will override the value in the index template file. For more information, see [Create index]({{site.url}}{{site.baseurl}}/api-reference/index-apis/create-index/).
`number_of_replicas` | No | Integer | The number of replica shards that each primary shard should have on the destination MCdesk server. For example, if you have 4 primary shards and set `number_of_replicas` to `3`, then the index has 12 replica shards. This parameter is effective only when `template_file` is either explicitly provided in the sink configuration or built in. If this parameter is set, then it will override the value in the index template file. For more information, see [Create index]({{site.url}}{{site.baseurl}}/api-reference/index-apis/create-index/).
`distribution_version` | No | String  | Indicates whether the backend version of the sink is Elasticsearch 6 or later. `es6` represents Elasticsearch 6. `default` represents the latest compatible backend version, such as Elasticsearch 7.x, MCdesk 1.x, or MCdesk 2.x. Default is `default`.
`enable_request_compression` | No | Boolean | Whether to enable compression when sending requests to MCdesk. When `distribution_version` is set to `es6`, default is `false`. For all other distribution versions, default is `true`.
`action` | No | String | The MCdesk bulk action to use for documents. Must be one of `create`, `index`, `update`, `upsert`, or `delete`. Default is `index`.
...
`normalize_index` | No | Boolean | If true, then the MCdesk sink will try to create dynamic index names. Index names with format options specified in `${})` are valid according to the [index naming restrictions]({{site.url}}{{site.baseurl}}/api-reference/index-apis/create-index/#index-naming-restrictions). Any invalid characters will be removed. Default value is `false`.
`routing` | No | String | A string used as a hash for generating the `shard_id` for a document when it is stored in MCdesk. Each incoming record is searched. When present, the string is used as the routing field for the document. When not present, the default routing mechanism (`document_id`) is used by MCdesk when storing the document. Supports formatting with fields in events and [Data Prepper expressions]({{site.url}}{{site.baseurl}}/data-prepper/pipelines/expression-syntax/), such as `${/my_field}-test-${getMetadata(\"some_metadata_key\")}`.
...
`serverless` | No | Boolean | **Deprecated in Data Prepper 2.7. Use this option with the `aws` configuration instead.** Determines whether the MCdesk backend is Amazon MCdesk Serverless. Set this value to `true` when the destination for the `mcdesk` sink is an Amazon MCdesk Serverless collection. Default is `false`.
`serverless_options` | No | Object | **Deprecated in Data Prepper 2.7. Use this option with the `aws` configuration instead.** The network configuration options available when the backend of the `mcdesk` sink is set to Amazon MCdesk Serverless. For more information, see [Serverless options](#serverless-options).
...
`serverless` | No | Boolean | Determines whether the MCdesk backend is Amazon MCdesk Serverless. Set this value to `true` when the destination for the `mcdesk` sink is an Amazon MCdesk Serverless collection. Default is `false`.
`serverless_options` | No | Object | The network configuration options available when the backend of the `mcdesk` sink is set to Amazon MCdesk Serverless. For more information, see [Serverless options](#serverless-options).
...
`when` | No | String | A [Data Prepper expression]({{site.url}}{{site.baseurl}}/data-prepper/pipelines/expression-syntax/) that conditionally evaluates whether an event will be sent to MCdesk using the bulk action configured in `type`. When empty, the bulk action will be chosen automatically when the event is sent to MCdesk.
...
The MCdesk sink acknowledges events only when:
* Successfully sent to MCdesk.
...
In order to send data to an MCdesk cluster using the `mcdesk` sink plugin, you must specify your username and password within the pipeline configuration. The following example `pipelines.yaml` file demonstrates how to specify admin security credentials:
...
If your MCdesk Service domain uses [fine-grained access control](https://docs.aws.amazon.com/mcdesk-service/latest/developerguide/fgac.html), the `mcdesk` sink plugin requires some additional configuration.
...
For more information, see [Recommended configurations](https://docs.aws.amazon.com/mcdesk-service/latest/developerguide/fgac.html#fgac-recommendations) in the Amazon MCdesk Service documentation.
...
The `mcdesk` sink plugin can send data to an [Amazon MCdesk Serverless](https://docs.aws.amazon.com/mcdesk-service/latest/developerguide/serverless.html) collection.

MCdesk Serverless collection sinks have the following limitations:
...
- The following [data access policy](https://docs.aws.amazon.com/mcdesk-service/latest/developerguide/serverless-data-access.html), which grants the required permissions to the pipeline role:
...
Within your `pipelines.yaml` file, specify the MCdesk Serverless collection endpoint as the `hosts` option. In addition, you must set the `serverless` option to `true`. Specify the pipeline role in the `sts_role_arn` option:
