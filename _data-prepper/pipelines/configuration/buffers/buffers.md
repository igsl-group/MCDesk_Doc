...
The `buffer` component acts as an intermediary layer between the `source` and `sink` components in an SmartObserve Data Prepper pipeline. It serves as temporary storage for events, decoupling the `source` from the downstream processors and sinks. Buffers can be either in-memory or disk based. 
