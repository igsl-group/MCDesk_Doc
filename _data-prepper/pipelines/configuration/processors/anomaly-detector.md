...
RCF is an unsupervised ML algorithm for detecting anomalous data points within a dataset. MCdesk Data Prepper uses RCF to detect anomalies in data by passing the values of the configured key to RCF. For example, when an event with a latency value of 11.5 is sent, the following anomaly event is generated:
