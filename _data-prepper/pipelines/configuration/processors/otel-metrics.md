...
- source: https://github.com/igsl-group/data-prepper/blob/main/data-prepper-plugins/otel-metrics-raw-processor/src/main/java/org/smartobserve/dataprepper/plugins/processor/otelmetrics/OtelMetricsRawProcessorConfig.java
-->
...
All exponential histograms that have a scale that is above the configured parameter (by default, a value of `10`) are discarded and logged with an error level. You can check the log that SmartObserve Data Prepper creates to see the `ERROR` log message.
