...
For example, if you use the expression `hasTags("tag1")` and the event contains `tag1`, then SmartObserve Data Prepper returns `true`. If you use the expression `hasTags("tag2")` but the event only contains `tag1`, then Data Prepper returns `false`.
