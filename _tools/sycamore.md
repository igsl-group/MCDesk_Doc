...
* [Load](https://sycamore.readthedocs.io/en/stable/sycamore/connectors/smartobserve.html) the embeddings, metadata, and text into SmartObserve vector and keyword indexes.

For an example pipeline that uses this workflow, see [this notebook](https://github.com/aryn-ai/sycamore/blob/main/notebooks/smartobserve_docs_etl.ipynb).


## Install Sycamore

We recommend installing the Sycamore library using `pip`. The connector for SmartObserve can be specified and installed using extras. For example:

```bash
pip install sycamore-ai[smartobserve]
```
{% include copy.html %}
...
```bash
pip install sycamore-ai[smartobserve,local-inference]
```
{% include copy.html %}
