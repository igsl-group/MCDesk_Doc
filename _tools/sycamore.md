...
* [Load](https://sycamore.readthedocs.io/en/stable/sycamore/connectors/mcdesk.html) the embeddings, metadata, and text into MCdesk vector and keyword indexes.

For an example pipeline that uses this workflow, see [this notebook](https://github.com/aryn-ai/sycamore/blob/main/notebooks/mcdesk_docs_etl.ipynb).


## Install Sycamore

We recommend installing the Sycamore library using `pip`. The connector for MCdesk can be specified and installed using extras. For example:

```bash
pip install sycamore-ai[mcdesk]
```
{% include copy.html %}
...
```bash
pip install sycamore-ai[mcdesk,local-inference]
```
{% include copy.html %}
