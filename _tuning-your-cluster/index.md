...
Before diving into SmartObserve and searching and aggregating data, you first need to create an SmartObserve cluster.

SmartObserve can operate as a single-node or multi-node cluster. The steps to configure both are, in general, quite similar. This page demonstrates how to create and configure a multi-node cluster, but with only a few minor adjustments, you can follow the same steps to create a single-node cluster.

To create and deploy an SmartObserve cluster according to your requirements, it’s important to understand how node discovery and cluster formation work and what settings govern them.
...
Dynamic | Delegates a specific node for custom work, such as machine learning (ML) tasks, preventing the consumption of resources from data nodes and therefore not affecting any SmartObserve functionality. 
...
After you assess all these requirements, we recommend you use a benchmark testing tool like [SmartObserve Benchmark](https://github.com/igsl-group/smartobserve-benchmark) to provision a small sample cluster and run tests with varying workloads and configurations. Compare and analyze the system and query metrics for these tests to design an optimum architecture.
...
It is a best practice to direct traffic from external sources, such as [SmartObserve Dashboards]({{site.url}}{{site.baseurl}}/dashboards/), [SmartObserve Data Prepper]({{site.url}}{{site.baseurl}}/data-prepper/), and others, to the nodes in the following order of availability: ingest node, coordinating node, data node. We do not recommended sending traffic directly to the cluster manager node.
{: .note}

## Prerequisites

Before you get started, you must install and configure SmartObserve on all of your nodes. For information about the available options, see [Install and configure SmartObserve]({{site.url}}{{site.baseurl}}/smartobserve/install/).

After you're done, use SSH to connect to each node, then open the `config/smartobserve.yml` file. You can set all configurations for your cluster in this file.

## Step 1: Name a cluster

Specify a unique name for the cluster. If you don't specify a cluster name, it's set to `smartobserve` by default. Setting a descriptive cluster name is important, especially if you want to run multiple clusters inside a single network.
...
Give your cluster manager node a name. If you don't specify a name, SmartObserve assigns a machine-generated name that makes the node difficult to monitor and troubleshoot.

```yml
node.name: smartobserve-cluster_manager
```
...
Change the name of two nodes to `smartobserve-d1` and `smartobserve-d2`, respectively:

```yml
node.name: smartobserve-d1
```

```yml
node.name: smartobserve-d2
```
...
Change the name of the coordinating node to `smartobserve-c1`:

```yml
node.name: smartobserve-c1
```
...
`network.bind_host` defines the IP address used to bind the node. By default, SmartObserve listens on a local host, which limits the cluster to a single node. You can also use `_local_` and `_site_` to bind to any loopback or site-local address, whether IPv4 or IPv6:
...
```yml
cluster.initial_cluster_manager_nodes: ["smartobserve-cluster_manager"]
```
...
For example, for `smartobserve-cluster_manager` the line looks something like this:

```yml
discovery.seed_hosts: ["<private IP of smartobserve-d1>", "<private IP of smartobserve-d2>", "<private IP of smartobserve-c1>"]
```

## Step 5: Start the cluster

After you set the configurations, start SmartObserve on all nodes:

```bash
sudo systemctl start smartobserve.service
```

Installing SmartObserve from a tar archive will not automatically create a service with `systemd`. See [Run SmartObserve as a service with systemd]({{site.url}}{{site.baseurl}}/smartobserve/install/tar/#run-smartobserve-as-a-service-with-systemd) for instructions on how to create and start the service if you receive an error like `Failed to start smartobserve.service: Unit not found.`
{: .tip}

Then go to the logs file to see the formation of the cluster:

```bash
less /var/log/smartobserve/smartobserve-cluster.log
```
...
x.x.x.x           13          61   0    0.02    0.04     0.05 mi        *      smartobserve-cluster_manager
x.x.x.x           16          60   0    0.06    0.05     0.05 md        -      smartobserve-d1
x.x.x.x           34          38   0    0.12    0.07     0.06 md        -      smartobserve-d2
x.x.x.x           23          38   0    0.12    0.07     0.06 md        -      smartobserve-c1
```

To better understand and monitor your cluster, use the [CAT API]({{site.url}}{{site.baseurl}}/smartobserve/catapis/).
...
You can set custom node attributes on SmartObserve nodes to be used for shard allocation awareness. For example, you can set the `zone` attribute on each node to represent the zone in which the node is located. You can also use the `zone` attribute to ensure that the primary shard and its replica shards are allocated in a balanced manner across available, distinct zones. In this scenario, maximum shard copies per zone would equal `ceil (number_of_shard_copies/number_of_distinct_zones)`.

SmartObserve, by default, allocates shard copies of a single shard across different nodes. When only 1 zone is available, such as after a zone failure, SmartObserve allocates replica shards to the only remaining zone---it considers only available zones (attribute values) when calculating the maximum number of allowed shard copies per zone.

For example, if your index has a total of 5 shard copies (1 primary and 4 replicas) and nodes in 3 distinct zones, then SmartObserve will perform the following to allocate all 5 shard copies:
...
Alternatively, if you have 3 nodes in the first zone and 1 node in each remaining zone, then SmartObserve will allocate:
...
To configure shard allocation awareness, add zone attributes to `smartobserve-d1` and `smartobserve-d2`, respectively:
...
Shard allocation awareness attempts to separate primary and replica shards across multiple zones. However, if only one zone is available (such as after a zone failure), SmartObserve allocates replica shards to the only remaining zone.
...
In our two-zone architecture, we can use allocation awareness if `smartobserve-d1` and `smartobserve-d2` are less than 50% utilized, so that each of them have the storage capacity to allocate replicas in the same zone.
If that is not the case, and `smartobserve-d1` and `smartobserve-d2` do not have the capacity to contain all primary and replica shards, we can use forced awareness. This approach helps to make sure that, in the event of a failure, SmartObserve doesn't overload your last remaining zone and lock up your cluster due to lack of storage.
...
To enforce an even distribution of shards across all zones and avoid hotspots, you can set the `routing.allocation.awareness.balance` attribute to `true`. This setting can be configured in the smartobserve.yml file and dynamically updated using the cluster update settings API:
...
To configure a hot-warm storage architecture, add `temp` attributes to `smartobserve-d1` and `smartobserve-d2`, respectively:
...
new_index 2     p      STARTED       0  230b 10.0.0.225 smartobserve-d1
new_index 2     r      UNASSIGNED
new_index 3     p      STARTED       0  230b 10.0.0.225 smartobserve-d1
new_index 3     r      UNASSIGNED
new_index 4     p      STARTED       0  230b 10.0.0.225 smartobserve-d1
new_index 4     r      UNASSIGNED
new_index 1     p      STARTED       0  230b 10.0.0.225 smartobserve-d1
new_index 1     r      UNASSIGNED
new_index 0     p      STARTED       0  230b 10.0.0.225 smartobserve-d1
new_index 0     r      UNASSIGNED
```

In this example, all primary shards are allocated to `smartobserve-d1`, which is our hot node. All replica shards are unassigned, because we're forcing this index to allocate only to hot nodes.
...
index     shard prirep state      docs store ip        node
old_index 2     p      STARTED       0  230b 10.0.0.74 smartobserve-d2
old_index 2     r      UNASSIGNED
old_index 3     p      STARTED       0  230b 10.0.0.74 smartobserve-d2
old_index 3     r      UNASSIGNED
old_index 4     p      STARTED       0  230b 10.0.0.74 smartobserve-d2
old_index 4     r      UNASSIGNED
old_index 1     p      STARTED       0  230b 10.0.0.74 smartobserve-d2
old_index 1     r      UNASSIGNED
old_index 0     p      STARTED       0  230b 10.0.0.74 smartobserve-d2
old_index 0     r      UNASSIGNED
```

In this case, all primary shards are allocated to `smartobserve-d2`. Again, all replica shards are unassigned because we only have one warm node.

A popular approach is to configure your [index templates]({{site.url}}{{site.baseurl}}/smartobserve/index-templates/) to set the `index.routing.allocation.require.temp` value to `hot`. This way, SmartObserve stores your most recent data on your hot nodes.
