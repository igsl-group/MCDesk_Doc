...
This tutorial shows you how to use the [`expand-data-corpus.py`](https://github.com/igsl-group/smartobserve-benchmark/blob/main/scripts/expand-data-corpus.py) script to increase the size of the data corpus for an SmartObserve Benchmark workload. This can be helpful when running the `http_logs` workload against a large SmartObserve cluster.
...
2. The `http_logs` workload data corpus is already stored on the load generation host running SmartObserve Benchmark.

## Understanding the script

The `expand-data-corpus.py` script is designed to generate a larger data corpus by duplicating and modifying existing documents from the `http_logs` workload corpus. It primarily adjusts the timestamp field while keeping other fields intact. It also generates an offset file, which enables SmartObserve Benchmark to start up faster.
...
You can generate multiple corpora by running the script multiple times with different output suffixes. All corpora generated by the script are used by SmartObserve Benchmark sequentially during injection.

## Verifying the documents

After the script completes, check the following locations for new files:

- In the SmartObserve Benchmark data directory for `http_logs`:
...
To use the newly generated corpus in an SmartObserve Benchmark test, use the following syntax:

```bash
smartobserve-benchmark run --workload http_logs --workload-params=generated_corpus:t [other_options]
```

The `generated_corpus:t` parameter tells SmartObserve Benchmark to use the expanded corpus. Any additional workload parameters can be appended using commas in the `--workload-params` option.
