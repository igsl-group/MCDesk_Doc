...
# node's mcdesk.yml file, and you will need to add entries
# for the additional nodes in mcdesk_dashboards.yml.
os_node_count=4
...
create_network()	{
	docker network create --subnet=172.20.0.0/16 mcdesk-dev-net || true
}	
...
# Write config files for MCdesk nodes.
write_os_configs ()	{
	for ((i=1; i <= $os_node_count; i++)); do
		cat <<- EOF > mcdesk-0${i}.yml
		---
		# Node and cluster config
		cluster.name: mcdesk-dev-cluster
		...
		path.repo: /usr/share/mcdesk/snapshots
		network.host: 0.0.0.0

		# Security plugin
		plugins.security.ssl.transport.pemcert_filepath: /usr/share/mcdesk/config/os-node-0${i}.pem
		plugins.security.ssl.transport.pemkey_filepath: /usr/share/mcdesk/config/os-node-0${i}-key.pem
		plugins.security.ssl.transport.pemtrustedcas_filepath: /usr/share/mcdesk/config/root-ca.pem
		plugins.security.ssl.http.enabled: true
		plugins.security.ssl.http.pemcert_filepath: /usr/share/mcdesk/config/os-node-0${i}.pem
		plugins.security.ssl.http.pemkey_filepath: /usr/share/mcdesk/config/os-node-0${i}-key.pem
		plugins.security.ssl.http.pemtrustedcas_filepath: /usr/share/mcdesk/config/root-ca.pem
...
# Write config file for MCdesk Dashboards.
write_osd_configs ()	{
	cat <<- 'EOF' > mcdesk_dashboards.yml
	---
	server.host: "0.0.0.0"
	server.name: "mcdesk-dashboards-dev"
	mcdesk.hosts: ["https://172.20.0.11:9200","https://172.20.0.12:9200","https://172.20.0.13:9200","https://172.20.0.14:9200"]
	...
	server.ssl.certificate: /usr/share/mcdesk-dashboards/config/os-dashboards-01.pem
	server.ssl.key: /usr/share/mcdesk-dashboards/config/os-dashboards-01-key.pem
	mcdesk.ssl.certificateAuthorities: ["/usr/share/mcdesk-dashboards/config/root-ca.pem"]
...
snapshot_repo_init()	{
	docker run --rm -v repo-01:/usr/share/mcdesk/snapshots busybox \
  		/bin/sh -c 'chown -R 1000:1000 /usr/share/mcdesk/snapshots'
}
...
launch_nodes()	{
	for ((i=1; i <= $os_node_count; i++)); do
		docker run -d \
			...
			-v data-0${i}:/usr/share/mcdesk/data \
		  	-v repo-01:/usr/share/mcdesk/snapshots \
		  	-v ~/deploy/mcdesk-0${i}.yml:/usr/share/mcdesk/config/mcdesk.yml \
		  	...
		  	-v ~/deploy/os-node-0${i}.pem:/usr/share/mcdesk/config/os-node-0${i}.pem \
		  	-v ~/deploy/os-node-0${i}-key.pem:/usr/share/mcdesk/config/os-node-0${i}-key.pem \
			--network mcdesk-dev-net \
			--ip 172.20.0.1${i} \
			--name os-node-0${i} \
			mcdeskproject/mcdesk:1.3.7
	done
}
...
launch_node_dashboards()	{
	docker run -d \
		...
		mcdeskproject/mcdesk-dashboards:1.3.7
}
