...
# node's smartobserve.yml file, and you will need to add entries
# for the additional nodes in smartobserve_dashboards.yml.
os_node_count=4
...
create_network()	{
	docker network create --subnet=172.20.0.0/16 smartobserve-dev-net || true
}	
...
create_certs()	{
	...
	# Create SmartObserve node certs.
	...
	# Create SmartObserve Dashboards cert.
...
# Write config files for SmartObserve nodes.
write_os_configs ()	{
	for ((i=1; i <= $os_node_count; i++)); do
		cat <<- EOF > smartobserve-0${i}.yml
		---
		# Node and cluster config
		cluster.name: smartobserve-dev-cluster
		...
		path.repo: /usr/share/smartobserve/snapshots
		network.host: 0.0.0.0

		# Security plugin
		plugins.security.ssl.transport.pemcert_filepath: /usr/share/smartobserve/config/os-node-0${i}.pem
		plugins.security.ssl.transport.pemkey_filepath: /usr/share/smartobserve/config/os-node-0${i}-key.pem
		plugins.security.ssl.transport.pemtrustedcas_filepath: /usr/share/smartobserve/config/root-ca.pem
		plugins.security.ssl.http.enabled: true
		plugins.security.ssl.http.pemcert_filepath: /usr/share/smartobserve/config/os-node-0${i}.pem
		plugins.security.ssl.http.pemkey_filepath: /usr/share/smartobserve/config/os-node-0${i}-key.pem
		plugins.security.ssl.http.pemtrustedcas_filepath: /usr/share/smartobserve/config/root-ca.pem
...
# Write config file for SmartObserve Dashboards.
write_osd_configs ()	{
	cat <<- 'EOF' > smartobserve_dashboards.yml
	---
	server.host: "0.0.0.0"
	server.name: "smartobserve-dashboards-dev"
	smartobserve.hosts: ["https://172.20.0.11:9200","https://172.20.0.12:9200","https://172.20.0.13:9200","https://172.20.0.14:9200"]
	...
	server.ssl.certificate: /usr/share/smartobserve-dashboards/config/os-dashboards-01.pem
	server.ssl.key: /usr/share/smartobserve-dashboards/config/os-dashboards-01-key.pem
	smartobserve.ssl.certificateAuthorities: ["/usr/share/smartobserve-dashboards/config/root-ca.pem"]
...
snapshot_repo_init()	{
	docker run --rm -v repo-01:/usr/share/smartobserve/snapshots busybox \
  		/bin/sh -c 'chown -R 1000:1000 /usr/share/smartobserve/snapshots'
}
...
launch_nodes()	{
	for ((i=1; i <= $os_node_count; i++)); do
		docker run -d \
			...
			-v data-0${i}:/usr/share/smartobserve/data \
		  	-v repo-01:/usr/share/smartobserve/snapshots \
		  	-v ~/deploy/smartobserve-0${i}.yml:/usr/share/smartobserve/config/smartobserve.yml \
		  	...
		  	-v ~/deploy/os-node-0${i}.pem:/usr/share/smartobserve/config/os-node-0${i}.pem \
		  	-v ~/deploy/os-node-0${i}-key.pem:/usr/share/smartobserve/config/os-node-0${i}-key.pem \
			--network smartobserve-dev-net \
			--ip 172.20.0.1${i} \
			--name os-node-0${i} \
			smartobserveproject/smartobserve:1.3.7
	done
}
...
launch_node_dashboards()	{
	docker run -d \
		...
		smartobserveproject/smartobserve-dashboards:1.3.7
}
