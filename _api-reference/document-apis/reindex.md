...
redirect_from: 
  ...
  - /smartobserve/rest-api/document-apis/reindex/
---
...
:--- | :--- | :---
refresh | Boolean | If true, SmartObserve refreshes shards to make the reindex operation available to search results. Valid options are `true`, `false`, and `wait_for`, which tells SmartObserve to wait for a refresh before executing the operation. Default is `false`.
timeout | Time | How long to wait for a response from the cluster. Default is `30s`.
wait_for_active_shards | String | The number of active shards that must be available before SmartObserve processes the reindex request. Default is 1 (only the primary shard). Set to `all` or a positive integer. Values greater than 1 require replicas. For example, if you specify a value of 3, the index must have two replicas distributed across two additional nodes for the operation to succeed.
wait_for_completion | Boolean | If `false`, SmartObserve runs the reindex operation asynchronously, without waiting for it to complete. The request returns immediately, and the task continues in the background. You can monitor its progress using the [Tasks API]({{site.url}}{{site.baseurl}}/api-reference/tasks/). Default is `true`, which means the operation runs synchronously.
...
slices | Integer | Number of sub-tasks SmartObserve should divide this task into. Default is 1, which means SmartObserve should not divide this task. Setting this parameter to `auto` indicates to SmartObserve that it should automatically decide how many slices to split the task into.
...
Field | Description
:--- | :---
conflicts | Indicates to SmartObserve what should happen if the Reindex operation runs into a version conflict. Valid options are `abort` and `proceed`. Default is `abort`.
...
remote | Information about a remote SmartObserve cluster to copy data from. Valid fields are `host`, `username`, `password`, `socket_timeout`, and `connect_timeout`.
host | Host URL of the SmartObserve cluster to copy data from.
...
slice | Whether to manually or automatically slice the reindex operation so it executes in parallel. Setting this field to `auto` allows SmartObserve to control the number of slices to use, which is one slice per shard, up to a maximum of 20. If there are multiple sources, the number of slices used are based on the index or backing index with the smallest number of shards.
...
script | A script that SmartObserve uses to apply transformations to the data during the reindex operation.
...
noops | How many documents SmartObserve ignored during the operation.
...
throttled_until_millis | The amount of time until SmartObserve executes the next throttled request.
