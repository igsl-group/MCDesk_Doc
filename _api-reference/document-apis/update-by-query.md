...
nav_order: 40
redirect_from: 
 - /smartobserve/rest-api/document-apis/update-by-query/
---
...
You can include a query and a script as part of your update request so SmartObserve can run the script to update all of the documents that match the query.
...
conflicts | String | Indicates to SmartObserve what should happen if the update by query operation runs into a version conflict. Valid options are `abort` and `proceed`. Default is `abort`.
...
preference | String | Specifies which shard or node SmartObserve should perform the update by query operation on.
...
refresh | Boolean | If true, SmartObserve refreshes shards to make the update by query operation available to search results. Valid options are `true` and `false`. Default is `false`.
...
slices | String or integer | The number slices to split an operation into for faster processing, specified by integer. When set to `auto` SmartObserve it should decides how many the number of slices for the operation. Default is `1`, which indicates an operation will not be split.
...
wait_for_completion | Boolean | When set to `false`, the response body includes a task ID and SmartObserve executes the operation asynchronously. The task ID can be used to check the status of the task or to cancel the task. Default is set to `true`.
...
To update your indexes and documents by query, you must include a [query]({{site.url}}{{site.baseurl}}/smartobserve/query-dsl/index) and a script in the request body that SmartObserve can run to update your documents. If you don't specify a query, then every document in the index gets updated.
...
noops | How many update requests SmartObserve ignored during the operation. This field always returns 0.
...
throttled_until_millis | The amount of time until SmartObserve executes the next throttled request. Always equal to 0 in an update by query request.
