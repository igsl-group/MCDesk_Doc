...
nav_order: 50
redirect_from:
 - /mcdesk/rest-api/document-apis/delete-by-query/
---
...
You can include a query as part of your delete request so MCdesk deletes all documents that match that query.
...
:--- | :--- | :--- | :---
&lt;index&gt; | String | Name or list of the data streams, indexes, or aliases to delete from. Supports wildcards. If left blank, MCdesk searches all indexes.
...
conflicts | String | Indicates to MCdesk what should happen if the delete by query operation runs into a version conflict. Valid options are `abort` and `proceed`. Default is `abort`.
...
preference | String | Specifies which shard or node MCdesk should perform the delete by query operation on.
...
refresh | Boolean | If true, MCdesk refreshes shards to make the delete by query operation available to search results. Valid values are `true` and `false`. Default is `false`.
...
search_timeout | Time | How long to wait until MCdesk deems the request timed out. Default is no timeout.
slices | String or Integer | How many slices to cut the operation into for faster processing. Specify an integer to set how many slices to divide the operation into, or use `auto`, which tells MCdesk it should decide how many slices to divide into. If you have a lot of shards in your index, set a lower number for better efficiency. Default is 1, which means the task should not be divided.
...
wait_for_active_shards | String | The number of shards that must be active before MCdesk executes the operation. Valid values are `all` or any integer up to the total number of shards in the index. Default is 1, which is the primary shard.
wait_for_completion | Boolean | Setting this parameter to false indicates to MCdesk it should not wait for completion and perform this request asynchronously. Asynchronous requests run in the background, and you can use the [Tasks]({{site.url}}{{site.baseurl}}/api-reference/tasks) API to monitor progress.


## Request body fields

To search your index for specific documents, you must include a [query]({{site.url}}{{site.baseurl}}/mcdesk/query-dsl/index) in the request body that MCdesk uses to match documents. If you don't use a query, MCdesk treats your delete request as a simple [delete document operation]({{site.url}}{{site.baseurl}}/api-reference/document-apis/delete-document).
...
Field | Description
:--- | :---
took | The amount of time in milliseconds MCdesk needed to complete the operation.
...
noops | How many delete requests MCdesk ignored during the operation. This field always returns 0.
...
throttled_until_millis | The amount of time until MCdesk executes the next throttled request. Always equal to 0 in a delete by query request.
