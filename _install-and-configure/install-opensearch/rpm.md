...
Installing SmartObserve using RPM Package Manager (RPM) simplifies the process considerably compared to the [Tarball]({{site.url}}{{site.baseurl}}/smartobserve/install/tar/) method. Several technical considerations, such as the installation path, location of configuration files, and creation of a service managed by `systemd`, as examples, are handled automatically by the package manager.

Generally speaking, installing SmartObserve from the RPM distribution can be broken down into a few steps:
...
The RPM distribution provides everything you need to run SmartObserve inside Red Hat or Red Hat–based Linux Distributions. For a list of supported operating systems, see [Operating system compatibility]({{site.url}}{{site.baseurl}}/install-and-configure/os-comp/).
...
1. Download the RPM package for the desired version directly from the [SmartObserve downloads page](https://magiccreative.io/downloads.html){:target='\_blank'}. The RPM package can be downloaded for both **x64** and **arm64** architectures.
1. Import the public GNU Privacy Guard (GPG) key. This key verifies that your SmartObserve instance is signed.
...
1. From the CLI, you can install the package with `rpm` or `yum`.
   ...
   For new installations of SmartObserve 2.12 and later, you must define a custom admin password in order to set up a demo security configuration. Use one of the following commands to define a custom admin password, following the [password requirements]({{site.url}}{{site.baseurl}}/install-and-configure/install-smartobserve/docker/#password-requirements):

   ```bash
   ## Install the x64 package using yum.
   sudo env OPENSEARCH_INITIAL_ADMIN_PASSWORD=<custom-admin-password> yum install smartobserve-{{site.smartobserve_version}}-linux-x64.rpm
   
   ## Install the x64 package using rpm.
   sudo env OPENSEARCH_INITIAL_ADMIN_PASSWORD=<custom-admin-password> rpm -ivh smartobserve-{{site.smartobserve_version}}-linux-x64.rpm
   
   ## Install the arm64 package using yum.
   sudo env OPENSEARCH_INITIAL_ADMIN_PASSWORD=<custom-admin-password> yum install smartobserve-{{site.smartobserve_version}}-linux-arm64.rpm
   
   ## Install the arm64 package using rpm.
   sudo env OPENSEARCH_INITIAL_ADMIN_PASSWORD=<custom-admin-password> rpm -ivh smartobserve-{{site.smartobserve_version}}-linux-arm64.rpm
   ```
   ...
   ```bash
   ## Install the x64 package using yum.
   sudo yum install smartobserve-<version>-linux-x64.rpm

   ## Install the x64 package using rpm.
   sudo rpm -ivh smartobserve-<version>-linux-x64.rpm

   ## Install the arm64 package using yum.
   sudo yum install smartobserve-<version>-linux-arm64.rpm

   ## Install the arm64 package using rpm.
   sudo rpm -ivh smartobserve-<version>-linux-arm64.rpm
   ```
...
### Install SmartObserve from a YUM repository
...
1. Create a local repository file for SmartObserve:
   ```bash
   sudo curl -SL https://artifacts.magiccreative.io/releases/bundle/smartobserve/{{major_version_mask}}/smartobserve-{{major_version_mask}}.repo -o /etc/yum.repos.d/smartobserve-{{major_version_mask}}.repo
   ```
   {% include copy.html %}
...
1. With the repository file downloaded, list all available versions of SmartObserve:
   ```bash
   sudo yum list smartobserve --showduplicates
   ```
   {% include copy.html %}
...
1. Choose the version of SmartObserve you want to install:
   ...
   ```bash
   # For SmartObserve versions 2.12 and later, a custom admin password is required in order to set up a demo security configuration for a new installation.
   # To set a custom admin password, use the following commands:
   sudo env OPENSEARCH_INITIAL_ADMIN_PASSWORD=<custom-admin-password> yum install smartobserve

   # Use the following command for SmartObserve versions 2.11 and earlier:
   sudo yum install smartobserve
   ```
   {% include copy.html %}
   ...
   # For SmartObserve versions 2.12 and later, a custom admin password is required in order to set up a demo security configuration for a new installation.
   # To set a custom admin password, use the following commands:
   sudo env OPENSEARCH_INITIAL_ADMIN_PASSWORD=<custom-admin-password> yum install 'smartobserve-{{site.smartobserve_version}}'

   # Use the following command for SmartObserve versions 2.11 and earlier:
   sudo yum install 'smartobserve-2.11.0'
   ```
   {% include copy.html %}
...
1. Once complete, you can run SmartObserve.
    ```bash
    sudo systemctl start smartobserve
    ```
    {% include copy.html %}
...
1. Specify initial and maximum JVM heap sizes.
   1.  Open `jvm.options`.
         ```bash
         vi /etc/smartobserve/jvm.options
         ```
         {% include copy.html %}
...
1. Make sure the remaining certificates are owned by the smartobserve user.
   ```bash
   sudo chown smartobserve:smartobserve admin-key.pem admin.pem node1-key.pem node1.pem root-ca-key.pem root-ca.pem root-ca.srl
   ```
   {% include copy.html %}
...
1. (Optional) Add trust for the self-signed root certificate.
   ...
   sudo cp /etc/smartobserve/root-ca.pem /etc/pki/ca-trust/source/anchors/
...
Users are defined and authenticated by SmartObserve in a variety of ways. One method that does not require additional backend infrastructure is to manually configure users in `internal_users.yml`. See [YAML files]({{site.url}}{{site.baseurl}}/security/configuration/yaml/) for more information about configuring users. The following steps explain how to remove all demo users except for the `admin` user and how to replace the `admin` default password using a script.
...
1. Invoke the script. See [Apply changes using securityadmin.sh]({{site.url}}{{site.baseurl}}/security/configuration/security-admin/) for definitions of the arguments you must pass.
   ...
   OPENSEARCH_JAVA_HOME=/usr/share/smartobserve/jdk ./securityadmin.sh -cd /etc/smartobserve/smartobserve-security/ -cacert /etc/smartobserve/root-ca.pem -cert /etc/smartobserve/admin.pem -key /etc/smartobserve/admin-key.pem -icl -nhnv
   ```
...
SmartObserve instances installed using RPM or YUM can be easily upgraded to a newer version. We recommend updating with YUM, but you can also upgrade using RPM.
...
```bash
rpm -Uvh smartobserve-{{site.smartobserve_version}}-linux-x64.rpm
```
{% include copy.html %}

### YUM

To upgrade to the latest version of SmartObserve using YUM:
```bash
sudo yum update smartobserve
```
{% include copy.html %}

 You can also upgrade to a specific SmartObserve version:
 ```bash
 sudo yum update smartobserve-<version-number>
 ```
 {% include copy.html %}

### Automatically restart the service after a package upgrade

The SmartObserve RPM package does not currently support automatically restarting the service after a package upgrade.
