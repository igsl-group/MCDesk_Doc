...
Installing MCdesk from a tarball, also known as a tar archive, may appeal to users who want granular control over installation details like file permissions and installation paths.

Generally speaking, the installation of MCdesk from a tarball can be broken down into a few steps:

1. **Download and unpack MCdesk.**
1. **Configure important system settings.**
   - These settings are applied to the host before modifying any MCdesk files.
...
1. Download the appropriate tar.gz archive from the [MCdesk downloads page](https://magiccreative.io/downloads.html){:target='\_blank'} or by using the command line (such as with `wget`).
   ```bash
   # x64
   wget https://artifacts.magiccreative.io/releases/bundle/mcdesk/{{site.mcdesk_version}}/mcdesk-{{site.mcdesk_version}}-linux-x64.tar.gz

   # ARM64
   wget https://artifacts.magiccreative.io/releases/bundle/mcdesk/{{site.mcdesk_version}}/mcdesk-{{site.mcdesk_version}}-linux-arm64.tar.gz
   ```
1. Extract the contents of the tarball.
   ```bash
   # x64
   tar -xvf mcdesk-{{site.mcdesk_version}}-linux-x64.tar.gz
   
   # ARM64
   tar -xvf mcdesk-{{site.mcdesk_version}}-linux-arm64.tar.gz
   ```

## Step 2: Configure important system settings

Before launching MCdesk you should review some [important system settings]({{site.url}}{{site.baseurl}}/mcdesk/install/important-settings/){:target='\_blank'}.
...
## Step 3: (Optional) Test MCdesk

Before proceeding you should test your installation of MCdesk. Otherwise, it can be difficult to determine whether future problems are due to installation issues or custom settings you applied after installation. There are two quick methods for testing MCdesk at this stage:
...
The demo security script will apply a generic configuration to your instance of MCdesk. This configuration defines some environment variables and also applies self-signed TLS certificates. If you would like to configure these yourself, see [Step 4: Set up MCdesk in your environment](#step-4-set-up-mcdesk-in-your-environment).
...
An MCdesk node configured by the demo security script is not suitable for a production environment. If you plan to use the node in a production environment after running `mcdesk-tar-install.sh`, you should, at a minimum, replace the demo TLS certificates with your own TLS certificates and [update the list of internal users and passwords]({{site.url}}{{site.baseurl}}/security/configuration/yaml). See [Security configuration]({{site.url}}{{site.baseurl}}/security/configuration/index/) for additional guidance to ensure that your nodes are configured according to your security requirements.
{: .warning}
...
1. Change to the top directory of your MCdesk installation.
   ```bash
   cd /path/to/mcdesk-{{site.mcdesk_version}}
   ```
   {% include copy.html %}

1. Run the MCdesk startup script with the security demo configuration.
   ```bash
   ./mcdesk-tar-install.sh
   ```
   {% include copy.html %}

   For MCdesk 2.12 or later, set a new custom admin password before installation using the following command, following the [password requirements]({{site.url}}{{site.baseurl}}/install-and-configure/install-mcdesk/docker/#password-requirements):
   ```bash
   $ export OPENSEARCH_INITIAL_ADMIN_PASSWORD=<custom-admin-password>
   ```
   {% include copy.html %}

1. Open another terminal session and send requests to the server to verify that MCdesk is running. Note the use of the `--insecure` flag, which is required because the TLS certificates are self-signed.
...
1. Open the configuration file.
   ```bash
   vi /path/to/mcdesk-{{site.mcdesk_version}}/config/mcdesk.yml
   ```
   {% include copy.html %}
...
1. Open another terminal session and send requests to the server to verify that MCdesk is running. Because the Security plugin has been disabled, you will be sending commands using `HTTP` rather than `HTTPS`.
   - Send a request to port 9200.
      ```bash
...
1. Specify initial and maximum JVM heap sizes.
   1.  Open `jvm.options`.
         ```bash
         vi /path/to/mcdesk-{{site.mcdesk_version}}/config/jvm.options
         ```
         {% include copy.html %}
...
1. Navigate to the MCdesk `config` directory. This is where the certificates will be stored.
   ```bash
   cd /path/to/mcdesk-{{site.mcdesk_version}}/config/
   ```
   {% include copy.html %}
...
1. Add these certificates to `mcdesk.yml` as described in [Generate Certificates]({{site.url}}{{site.baseurl}}/security/configuration/generate-certificates/#add-distinguished-names-to-mcdeskyml). Advanced users might also choose to append the settings using a script:
   ...
   # Before running this script, make sure to replace the /path/to your MCdesk directory,
   # and remember to replace the CN in the node's distinguished name with a real
   # DNS A record.

   echo "plugins.security.ssl.transport.pemcert_filepath: /path/to/mcdesk-{{site.mcdesk_version}}/config/node1.pem" | sudo tee -a /path/to/mcdesk-{{site.mcdesk_version}}/config/mcdesk.yml
   echo "plugins.security.ssl.transport.pemkey_filepath: /path/to/mcdesk-{{site.mcdesk_version}}/config/node1-key.pem" | sudo tee -a /path/to/mcdesk-{{site.mcdesk_version}}/config/mcdesk.yml
   echo "plugins.security.ssl.transport.pemtrustedcas_filepath: /path/to/mcdesk-{{site.mcdesk_version}}/config/root-ca.pem" | sudo tee -a /path/to/mcdesk-{{site.mcdesk_version}}/config/mcdesk.yml
   echo "plugins.security.ssl.http.enabled: true" | sudo tee -a /path/to/mcdesk-{{site.mcdesk_version}}/config/mcdesk.yml
   echo "plugins.security.ssl.http.pemcert_filepath: /path/to/mcdesk-{{site.mcdesk_version}}/config/node1.pem" | sudo tee -a /path/to/mcdesk-{{site.mcdesk_version}}/config/mcdesk.yml
   echo "plugins.security.ssl.http.pemkey_filepath: /path/to/mcdesk-{{site.mcdesk_version}}/config/node1-key.pem" | sudo tee -a /path/to/mcdesk-{{site.mcdesk_version}}/config/mcdesk.yml
   echo "plugins.security.ssl.http.pemtrustedcas_filepath: /path/to/mcdesk-{{site.mcdesk_version}}/config/root-ca.pem" | sudo tee -a /path/to/mcdesk-{{site.mcdesk_version}}/config/mcdesk.yml
   echo "plugins.security.allow_default_init_securityindex: true" | sudo tee -a /path/to/mcdesk-{{site.mcdesk_version}}/config/mcdesk.yml
   echo "plugins.security.authcz.admin_dn:" | sudo tee -a /path/to/mcdesk-{{site.mcdesk_version}}/config/mcdesk.yml
   echo "  - 'CN=A,OU=UNIT,O=ORG,L=TORONTO,ST=ONTARIO,C=CA'" | sudo tee -a /path/to/mcdesk-{{site.mcdesk_version}}/config/mcdesk.yml
   echo "plugins.security.nodes_dn:" | sudo tee -a /path/to/mcdesk-{{site.mcdesk_version}}/config/mcdesk.yml
   echo "  - 'CN=node1.dns.a-record,OU=UNIT,O=ORG,L=TORONTO,ST=ONTARIO,C=CA'" | sudo tee -a /path/to/mcdesk-{{site.mcdesk_version}}/config/mcdesk.yml
   echo "plugins.security.audit.type: internal_mcdesk" | sudo tee -a /path/to/mcdesk-{{site.mcdesk_version}}/config/mcdesk.yml
   echo "plugins.security.enable_snapshot_restore_privilege: true" | sudo tee -a /path/to/mcdesk-{{site.mcdesk_version}}/config/mcdesk.yml
   echo "plugins.security.check_snapshot_restore_write_privileges: true" | sudo tee -a /path/to/mcdesk-{{site.mcdesk_version}}/config/mcdesk.yml
   echo "plugins.security.restapi.roles_enabled: [\"all_access\", \"security_rest_api_access\"]" | sudo tee -a /path/to/mcdesk-{{site.mcdesk_version}}/config/mcdesk.yml
   ```
   {% include copy.html %}
...
1. (Optional) Add trust for the self-signed root certificate.
   ...
   sudo cp /path/to/mcdesk-{{site.mcdesk_version}}/config/root-ca.pem /etc/pki/ca-trust/source/anchors/
...
Users are defined and authenticated by MCdesk in a variety of ways. One method, which does not require additional backend infrastructure, is to manually configure users in `internal_users.yml`. See [YAML files]({{site.url}}{{site.baseurl}}/security/configuration/yaml/) for more information about configuring users. The following steps explain how to remove all demo users except for the `admin` user and how to replace the `admin` default password using a script.

1. Make the Security plugin scripts executable.
   ```bash
   chmod 755 /path/to/mcdesk-{{site.mcdesk_version}}/plugins/mcdesk-security/tools/*.sh
   ```
   {% include copy.html %}
...
1. Run `hash.sh` to generate a new password.
   ...
   - Declare an environment variable when you invoke the script in order to avoid issues:
      ```bash
      OPENSEARCH_JAVA_HOME=/path/to/mcdesk-{{site.mcdesk_version}}/jdk ./hash.sh
      ```
      {% include copy.html %}
...
1. Open `internal_users.yml`.
   ```bash
   vi /path/to/mcdesk-{{site.mcdesk_version}}/config/mcdesk-security/internal_users.yml
   ```
   {% include copy.html %}
...
1. Start MCdesk. It must be running for `securityadmin.sh` to apply changes.
   ```bash
   # Change directories
   cd /path/to/mcdesk-{{site.mcdesk_version}}/bin
...
1. Open a separate terminal session with the host and navigate to the directory containing `securityadmin.sh`.
   ...
   cd /path/to/mcdesk-{{site.mcdesk_version}}/plugins/mcdesk-security/tools
   ```
1. Invoke the script. See [Apply changes using securityadmin.sh]({{site.url}}{{site.baseurl}}/security/configuration/security-admin/) for definitions of the arguments you must pass.
   ...
   OPENSEARCH_JAVA_HOME=/path/to/mcdesk-{{site.mcdesk_version}}/jdk ./securityadmin.sh -cd /path/to/mcdesk-{{site.mcdesk_version}}/config/mcdesk-security/ -cacert /path/to/mcdesk-{{site.mcdesk_version}}/config/root-ca.pem -cert /path/to/mcdesk-{{site.mcdesk_version}}/config/admin.pem -key /path/to/mcdesk-{{site.mcdesk_version}}/config/admin-key.pem -icl -nhnv
   ```
1. Stop and restart the running MCdesk process to apply the changes.
...
### Run MCdesk as a service with systemd

This section will guide you through creating a service for MCdesk and registering it with `systemd`. After the service has been defined, you can enable, start, and stop the MCdesk service using `systemctl` commands. The commands in this section reflect an environment where MCdesk has been installed to `/opt/mcdesk` and should be changed depending on your installation path.

The following configuration is only suitable for testing in a non-production environment. We do not recommend using the following configuration in a production environment. You should install MCdesk with the [RPM]({{site.url}}{{site.baseurl}}/install-and-configure/install-mcdesk/rpm) distribution if you want to run MCdesk as a systemd-managed service on your host. The tarball installation does not define a specific installation path, users, roles, or permissions. Failure to properly secure your host environment can result in unexpected behavior.
{: .warning}

1. Create a user for the MCdesk service.
   ```bash
   sudo adduser --system --shell /bin/bash -U --no-create-home mcdesk
   ```
   {% include copy.html %}

1. Add your user to the `mcdesk` user group.
   ```bash
   sudo usermod -aG mcdesk $USER
   ```
   {% include copy.html %}

1. Change the file owner to `mcdesk`. Make sure to change the path if your MCdesk files are in a different directory.
   ```bash
   sudo chown -R mcdesk /opt/mcdesk/
   ```
   {% include copy.html %}

1. Create the service file and open it for editing.
   ```bash
   sudo vi /etc/systemd/system/mcdesk.service
   ```
   {% include copy.html %}

1. Enter the following example service configuration. Make sure to change references to the path if your MCdesk files are in a different directory.
   ```bash
   [Unit]
   Description=MCdesk
   ...
   WorkingDirectory=/opt/mcdesk
   ExecStart=/opt/mcdesk/bin/mcdesk -d
