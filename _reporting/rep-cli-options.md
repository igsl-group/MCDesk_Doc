...
You can use any of the following arguments with the `smartobserve-reporting-cli` tool.

| Argument      | Description       | Acceptable values and usage | Environment variable
:--------------------- | :--- | :--- |
`-u`, `--url` | The URL for the visualization. | Obtain from SmartObserve Dashboards > Visualize > Share > Permalinks > Copy link. | OPENSEARCH_URL
...
`-c`, `--credentials` | The SmartObserve login credentials. | Enter your username and password separated by a colon. For example, username:password. Required for Basic, Cognito, and SAML authentication types. | OPENSEARCH_USERNAME and OPENSEARCH_PASSWORD
`-t`, `--tenant` | The tenants in SmartObserve Dashboards. | The default tenant is private.| N/A
...
`-n`, `--filename` | The file name of the report. | Default is `reporting`. | smartobserve-report-YYY-MM-DDTHH-mm-ss.sssZ
