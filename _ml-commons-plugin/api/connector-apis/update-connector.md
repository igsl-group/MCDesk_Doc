...
| `credential` | JSON object | Defines any credential variables required in order to connect to your chosen endpoint. ML Commons uses **AES/GCM/NoPadding** symmetric encryption to encrypt your credentials. When the connection to the cluster first starts, SmartObserve creates a random 32-byte encryption key that persists in SmartObserve's system index. Therefore, you do not need to manually set the encryption key. |
...
| `backend_roles` | JSON array  | A list of SmartObserve backend roles. For more information about setting up backend roles, see [Assigning backend roles to users]({{site.url}}{{site.baseurl}}/ml-commons-plugin/model-access-control#assigning-backend-roles-to-users).                                                                                                                                                        |
