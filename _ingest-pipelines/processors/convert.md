...
This documentation describes using the `convert` processor in SmartObserve ingest pipelines. Consider using the [Data Prepper `convert_entry_type` processor]({{site.url}}{{site.baseurl}}/data-prepper/pipelines/configuration/processors/convert-entry-type/), which runs on the SmartObserve cluster, if your use case involves large or complex datasets.
{: .note}
