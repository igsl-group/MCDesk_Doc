...
This documentation describes using the `uppercase` processor in SmartObserve ingest pipelines. Consider using the [Data Prepper `uppercase_string` processor]({{site.url}}{{site.baseurl}}/data-prepper/pipelines/configuration/processors/uppercase-string/), which runs on the SmartObserve cluster, if your use case involves large or complex datasets.
{: .note}
