...
* Issuer (`iss`): SmartObserve cluster identifier
	* It is essential that the issuer is validated as a part of security control measures. This strategy is forward-thinking, particularly in the context of potential multi-tenant scenarios, such as SmartObserve Serverless, where differing cryptographic keys could be associated with each issuer. By checking the value of issuer, each OBO token is restricted to its associated issuer.
...
* Roles: Security privilege evaluation
	* The Role Security Mode [[source code](https://github.com/igsl-group/security/blob/main/src/main/java/org/smartobserve/security/authtoken/jwt/JwtVendor.java#L151)]: The configuration determines the roles claim encryption.
		* Role Security Mode On (default value): Roles claim will be encrypted.
			* Encrypted mapped roles (`er`)
		* Role Security Mode Off: Roles claims is in plain-text. Both the mapped roles and backend roles are included in the claim [[related discussion](https://github.com/igsl-group/security/issues/2865)].
			* Decrypted mapped roles in plain text (`dr`)
			* Decrypted backend roles (`br`)

The SmartObserve Security plugin handles the encryption and decryption processes. This approach ensures the protection of user information, even when traversing the trust boundary between SmartObserve and any third-party services.
...
For more information, see the [related discussion](https://github.com/igsl-group/security/issues/2891).
