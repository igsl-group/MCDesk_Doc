...
This profile is meant for use with web browsers. It is not a general-purpose way of authenticating users against the Security plugin, so its primary use case is to support SmartObserve Dashboards single sign-on.


## Docker example

We provide a fully functional example that can help you understand how to use SAML with SmartObserve Dashboards.

1. Visit the [saml-demo branch](https://github.com/igsl-group/demos/tree/saml-demo) of the demos repository and download it to a folder of your choice. If you're not familiar with how to use GitHub, see the [SmartObserve onboarding guide](https://github.com/igsl-group/demos/blob/main/ONBOARDING.md) for instructions.
...
1. Review the following files, as needed:
   ...
   * `.env`: 
     * Defines the SmartObserve and SmartObserve Dashboards version to use. The default is the latest version ({{site.smartobserve_major_minor_version}}).
     * Defines the `OPENSEARCH_INITIAL_ADMIN_PASSWORD` variable required by versions 2.12 and later.
   * `./custom-config/smartobserve_dashboards.yml`: Includes the SAML settings for the default `smartobserve_dashboards.yml` file.
   * `./custom-config/config.yml`: Configures SAML for authentication.
   * `docker-compose.yml`: Defines an SmartObserve server node, an SmartObserve Dashboards server node, and a SAML server node.
...
1. Access SmartObserve Dashboards at [http://localhost:5601](http://localhost:5601){:target='\_blank'}.

1. Select `Log in with single sign-on`. This redirects you to the SAML login page.

1. Log in to SmartObserve Dashboards with a user defined in `./saml/config/authsources.php` (such as `user1` with password `user1pass`).
...
To use SAML for authentication, you need to configure a respective authentication domain in the `authc` section of `config/smartobserve-security/config.yml`. Because SAML works solely on the HTTP layer, you do not need any `authentication_backend` and can set it to `noop`. Place all SAML-specific configuration options in this chapter in the `config` section of the SAML HTTP authenticator:
...
After you have configured SAML in `config.yml`, you must also [activate it in SmartObserve Dashboards](#smartobserve-dashboards-configuration).


## Running multiple authentication domains

We recommend adding at least one other authentication domain, such as LDAP or the internal user database, to support API access to SmartObserve without SAML. For SmartObserve Dashboards and the internal SmartObserve Dashboards server user, you also must add another authentication domain that supports basic authentication. This authentication domain should be placed first in the chain, and the `challenge` flag must be set to `false`:
...
Name | Description
:--- | :---
`idp.metadata_file` | The path to the SAML 2.0 metadata file of your IdP. Place the metadata file in the `config` directory of SmartObserve. The path has to be specified relative to the `config` directory. Required if `idp.metadata_url` is not set.
...
An entity ID is a globally unique name for a SAML entity, either an IdP or a service provider (SP). The IdP entity ID is usually provided by your IdP. The SP entity ID is the name of the configured application or client in your IdP. We recommend adding a new application for SmartObserve Dashboards and using the URL of your SmartObserve Dashboards installation as the SP entity ID.
...
By default, SmartObserve Security allows for a window of 30 seconds to compensate for possible misalignment between server clock times. To set a custom value for this feature and override the default, you can add the `jwt_clock_skew_tolerance_seconds` setting to the `config.yml`.
...
## SmartObserve Dashboards settings

The web browser SSO profile exchanges information through HTTP GET or POST. For example, after you log in to your IdP, it sends an HTTP POST back to SmartObserve Dashboards containing the SAML response. You must configure the base URL of your SmartObserve Dashboards installation where the HTTP requests are being sent to.

Name | Description
:--- | :---
`kibana_url` | The SmartObserve Dashboards base URL. Required.
...
`sp.signature_private_key_filepath` | Path to the private key. The file must be placed under the SmartObserve `config` directory, and the path must be specified relative to that same directory.
...
Usually, IdPs provide information about their individual logout URL in their SAML 2.0 metadata. If this is the case, the Security plugin uses them to render the correct logout link in SmartObserve Dashboards. If your IdP does not support an explicit logout, you can force a re-login when the user visits SmartObserve Dashboards again.
...
Name | Description
:--- | :---
`idp.pemtrustedcas_filepath` | Path to the PEM file containing the root CAs of your IdP. The files must be placed under the SmartObserve `config` directory, and you must specify the path relative to that same directory.
...
`idp.pemcert_filepath` | Path to the PEM file containing the client certificate. The file must be placed under the SmartObserve `config` directory, and the path must be specified relative to the `config` directory.
`idp.pemcert_content` | The content of the client certificate. Cannot be used when `pemcert_filepath` is set.
`idp.pemkey_filepath` | Path to the private key of the client certificate. The file must be placed under the SmartObserve `config` directory, and the path must be specified relative to the `config` directory.
      ...
      saml_auth_domain:
        ...
        http_authenticator:
          ...
          config:
            ...
            sp:
              entity_id: https://smartobserve-dashboards.example.com
            kibana_url: https://smartobserve-dashboards.example.com:5601/
            roles_key: Role
...
Because most of the SAML-specific configuration is done in the Security plugin, just activate SAML in your `smartobserve_dashboards.yml` by adding the following:

```yml
smartobserve_security.auth.type: "saml"
```

In addition, you must add the SmartObserve Dashboards endpoint for validating the SAML assertions to your allow list:
...
```yml
smartobserve_security.saml.extra_storage.cookie_prefix: security_authentication_saml
smartobserve_security.saml.extra_storage.additional_cookies: 3
```
...
Then add this endpoint to `server.xsrf.allowlist` in `smartobserve_dashboards.yml`:
