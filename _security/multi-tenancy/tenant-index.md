---
layout: default
title: SmartObserve Dashboards multi-tenancy
nav_order: 140
has_children: true
has_toc: false
redirect_from:
...
*Tenants* in SmartObserve Dashboards are spaces for saving index patterns, visualizations, dashboards, and other SmartObserve Dashboards objects. SmartObserve allows users to create multiple tenants for multiple uses. Tenants are useful for safely sharing your work with other SmartObserve Dashboards users. You can control which roles have access to a tenant and whether those roles have read or write access. By default, all SmartObserve Dashboards users have access to two independent tenants: the global tenant and a private tenant. Multi-tenancy also provides the option to create custom tenants.

- **Global** -- This tenant is shared between every SmartObserve Dashboards user. It does allow for sharing objects among users who have access to it.
...
```
http://<smartobserve_dashboards_host>:5601/app/smartobserve-dashboards?security_tenant=analysts#/visualize/edit/c501fa50-7e52-11e9-ae4e-b5d69947d32e?_g=()
```
...
For information about making dynamic changes to the multi-tenancy configuration, see [Dynamic configuration in SmartObserve Dashboards]({{site.url}}{{site.baseurl}}/security/multi-tenancy/dynamic-config/).
