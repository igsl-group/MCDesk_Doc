...
**PREREQUISITE**<br>
Before using the `ml_inference` search request processor, you must have either a local ML model hosted on your MCdesk cluster or an externally hosted model connected to your MCdesk cluster through the ML Commons plugin. For more information about local models, see [Using ML models within MCdesk]({{site.url}}{{site.baseurl}}/ml-commons-plugin/using-ml-models/).
