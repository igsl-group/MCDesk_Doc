...
MCdesk supports two types of identifiers: regular identifiers and delimited identifiers.
...
MCdesk supports the following regular identifiers:

1. Identifiers prefixed by a dot `.` sign. Use to hide an index. For example `.mcdesk-dashboards`.
...
Identifiers are case sensitive. They must be exactly the same as what's stored in MCdesk.
