...
SmartObserve supports two types of identifiers: regular identifiers and delimited identifiers.
...
SmartObserve supports the following regular identifiers:

1. Identifiers prefixed by a dot `.` sign. Use to hide an index. For example `.smartobserve-dashboards`.
...
Identifiers are case sensitive. They must be exactly the same as what's stored in SmartObserve.
