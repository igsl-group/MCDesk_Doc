...
MCdesk relies on different on-heap cache types to accelerate data retrieval, providing significant improvement in search latency. However, cache size is limited by the amount of memory available on a node. When processing a larger dataset that can potentially be cached, the cache size limit can result in many pieces of data either being removed from the cache or not being cached, causing an incomplete query. This impacts performance because MCdesk needs to process the query again, causing high resource consumption.
...
MCdesk supports the following on-heap cache types:
...
In addition to existing custom MCdesk on-heap cache stores, cache plugins provide the following cache stores: 
...
In MCdesk 2.14, the request cache is integrated with cache plugins. You can use a tiered or disk cache as a request-level cache.
{: .note}
