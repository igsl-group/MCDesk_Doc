...
| Index pattern `mcdesk-forecast-results*` | Matches the default result index pattern. Update this pattern if you route results to a custom index, such as `mcdesk-forecast-result-abc*`. |
...
| Threshold | Use the MCdesk Dashboards visual editor to analyze recent forecast values and determine an appropriate threshold that reliably indicates anomalies. |
